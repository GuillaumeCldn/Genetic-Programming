...
def generate_population(family_size, mode=1):
    if mode == 1:
        ...
        type = 'G'
    init_pop = [ob.Tree(f'{type}{i}') for i in range(family_size)]
    if mode == 1:
...
def fitness(tree):
    # print(tree.evaluate.cache_info())
    print('fit')
...
def genetic():
    ...
    while fitness(best_tree2) > d.accuracy:
        ...
        for chosen in chosen_mutate:
            new_gen.append(ev.mutate(chosen))
        print('g_3')
        ...
        for i in range(num_fuse):
            ...
            new_gen.append(ev.fuse(parent1, parent2))
...
#  gr.draw_tree(d.testTree4)
#    print(f'{d.testTree4.nodes[3].data_type}')
#    new_tree = ev.mutate(d.testTree4)
#    gr.draw_tree(new_tree)
    if Go:
        profiler = cProfile.Profile()
        profiler.enable()
        bestTree, fitnesses = genetic()
        profiler.disable()

        gr.draw_tree(bestTree)
        gr.draw_function(bestTree, d.param_list, d.Y_ref)
        X2 = [i for i in range(len(fitnesses))]
        plt.plot(X2, fitnesses)
        plt.show()
